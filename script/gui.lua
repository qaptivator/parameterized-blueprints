...
gui.handle_select_signal_button_changed = function(event)
	...
	if
		event.element.elem_value and string.match(event.element.elem_value.name, blueprints.placeholder_pattern_string)
	then
		event.element.elem_value = nil
		game.players[event.player_index].print(
			"placeholder items are not valid here (these cannot be hidden from the ui)"
		)
		return
	end
...
gui.get_changes_from_ui = function(player_index)
	if not gui.is_options_open(player_index) then
		return nil
	end
